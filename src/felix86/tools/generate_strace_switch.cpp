// Generates initial strace boilerplate that we can then modify by hand to make better
#include <nlohmann/json.hpp>
#include <stdio.h>

extern const char* syscall_json64;
extern const char* syscall_json32;

constexpr unsigned long hashstr(const char* str, int h = 0) {
    return !str[h] ? 55 : (hashstr(str, h + 1) * 33) + (unsigned char)(str[h]);
}

std::string get_type(const std::string& arg) {
...
std::string get_format_specifier(const std::string& type) {
    switch (hashstr(get_type(type).c_str())) {
    case hashstr("unsigned int"):
    case hashstr("int"):
    case hashstr("unsigned long"):
    case hashstr("long"):
    case hashstr("off_t"):
    case hashstr("loff_t"):
    case hashstr("size_t"):
    case hashstr("ssize_t"):
        return "0x{:x}";
    case hashstr("void *"):
    case hashstr("void **"):
    case hashstr("const char *"):
    case hashstr("char *"):
        return "{}";
    }
...
std::string get_format_value(const std::string& type, int arg) {
    switch (hashstr(get_type(type).c_str())) {
    case hashstr("unsigned int"):
    case hashstr("int"):
        return "(int)arg" + std::to_string(arg);
    case hashstr("unsigned long"):
    case hashstr("long"):
    case hashstr("off_t"):
    case hashstr("loff_t"):
    case hashstr("size_t"):
    case hashstr("ssize_t"):
        return "(u64)arg" + std::to_string(arg);
    case hashstr("char *"):
    case hashstr("const char *"):
        return "(const char*)arg" + std::to_string(arg);
    }

    return "(void*)arg" + std::to_string(arg);
}

int main() {

    printf("// Generated by generate_strace_switch.cpp\n");
    printf("#include \"felix86/common/utility.hpp\"\n");
    printf("#include \"felix86/common/log.hpp\"\n");
    printf("#include <fmt/format.h>\n");
    printf("#include <string>\n\n");

    for (int i = 0; i < 2; i++) {
        nlohmann::ordered_json json = nlohmann::json::parse(i == 0 ? syscall_json32 : syscall_json64);
        auto& syscalls = json["syscalls"];
        printf("std::string trace%d(int syscall_no, u64 arg1, u64 arg2, u64 arg3, u64 arg4, u64 arg5, u64 arg6) {\n", 32 + i * 32);
        printf("    switch (syscall_no) {\n");
        for (int i = 0; i < syscalls.size(); i++) {
            auto& element = syscalls[i];
            auto& signature = element["signature"];
            auto& name = element["name"];
            printf("        case %d: {\n", element["index"].get<int>());
            printf("             return fmt::format(\"%s", name.get<std::string>().c_str());
            printf(": {{");
            for (int i = 0; i < signature.size(); i++) {
                auto& type = signature[i];
                printf("\\n");
                printf("    %s = %s", type.get<std::string>().c_str(), get_format_specifier(type.get<std::string>()).c_str());
            ...
            for (int i = 0; i < signature.size(); i++) {
                auto& type = signature[i];
                printf("%s", get_format_value(type.get<std::string>(), i + 1).c_str());
                if (i != signature.size() - 1) {
                    printf(", ");
                }
            }
        ...
        printf("        default: ERROR(\"Unknown syscall: %%d\", syscall_no); return \"\";\n");
        printf("    }\n");
        printf("}\n");
    }
}

const char* syscall_json64 = R"(
{
	"kernel": {
		"abi": {
			"bits": 64,
	...
	"systrack_version": "0.7-rc2"
}
)";

const char* syscall_json32 = R"(
    {
	"kernel": {
		"abi": {
			"bits": 32,
